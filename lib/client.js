...
module.exports = class Client extends EventEmitter {
  ...
  setupConfig(config) {
    ...
    if (!config.url) throw new ClientError.ConfigError('URI needs to be specified for client connection', this.CONSTANTS.ERRORS.CLIENT.CONNECT_BAD_URI);
  ...
  receive(message) {
    ...
    if (message.action == 'pub') return this.receiveSubscription(message);
  ...
  receiveResponse(message) {
    ...
    if (!this.receiveHandlers[message.payload.tx]) return this.emit('error', {
      reason: 'timed out server message',
      message: message
    });

    clearTimeout(this.receiveHandlers[message.payload.tx].timeout);

    try {
      this.receiveHandlers[message.payload.tx].handler(null, message);
    } catch (e) {
      this.emitEvent('error', {reason: 'response received handler failed', message: message, error: e});
    }

    delete this.receiveHandlers[message.payload.tx];
  }
  ...
  writeMessage(message) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.STATE != this.CONSTANTS.CLIENT_STATE.CONNECTED) throw new ClientError.ConnectError('Attempt to write when client disconnected', this.CONSTANTS.ERRORS.CLIENT.DISCONNECTED_WRITE);
      ...
      this.receiveHandlers[message.tx].timeout = setTimeout(() => {
        ...
        reject(new ClientError.TimeoutError('message timed out', this.CONSTANTS.ERRORS.CLIENT.RESPONSE_TIMEOUT, message));
        delete this.receiveHandlers[message.tx];
  ...
  async unsubscribe(subscription, options) {
    ...
    if (this.subscriptions[subscription.payload.topic].refCount == 0)
      await this.writeMessage(this.protocol.createMessage('unsubscribe', subscription, options));
