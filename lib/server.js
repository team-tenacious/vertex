...
module.exports = class Server {
  ...
  instantiateServices() {
    ...
    Object.keys(this.config.services).forEach(serviceName => {
      ...
      this.dependancyMap.add(serviceName, Service, Service.dependants);
    });
  ...
  defaults(){
    if (!this.config.services['subscription-cache']) this.config.services['subscription-cache'] = {};
    if (!this.config.services.subscription) this.config.services.subscription = {};
    if (!this.config.services.edge) this.config.services.edge = {};
    if (!this.config.services.sockets) this.config.services.sockets = {};
