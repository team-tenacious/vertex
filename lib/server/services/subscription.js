...
module.exports = class Subscription extends EventEmitter {
  ...
  onMessage(origin, message) {
    ...
    if (!this.actionHandlers[message.data.action]) return this.respondError(origin, message, new Error('No handler for action: ' + message.data.action));
  ...
  respondOK(origin, message, response){
    ...
    this.cluster.write(origin, this.protocol.createReply(message.data, {status:1, response: response}));
  }

  respondError(origin, message, e){
    this.emit('message-process-error', e);
    this.cluster.write(origin, this.protocol.createReply(message.data, {status:0, error: this.protocol.serializeError(e)}));
  }
