...
module.exports = class ws extends EventEmitter {
  ...
  writeConnectionConfirmation(sessionId){
    ...
    var confirmationMessage = protocol.createMessage('connection-confirmed', {server: {name: this.server.name, address: this.server.services.cluster.advertiseAddress}});
  ...
  async startWsService() {
    ...
    this.wss.on('connection', (ws, req) => {
      ...
      ws.on('message', (data) => {
        this.emit('message', {sessionId: ws.sessionId, data: utils.deserializeWsMessage(data, this.config.wsImplementation)});
      });
