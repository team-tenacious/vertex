...
module.exports = class ws extends EventEmitter {
  ...
  async start() {
    this.logger.info('starting ws service');

    await this.startWsService();

    this.logger.info('started ws service');
  }
  ...
  listClients(){
    ...
    return Object.keys(this.clients).sort().map((sessionId) => {
  ...
  async startWsService() {
    ...
    this.wss.on('connection', (ws, req) => {
      ...
      this.logger.info('new client session: ' + sessionId + ', ' + Object.keys(this.clients).length + ' client connections');
