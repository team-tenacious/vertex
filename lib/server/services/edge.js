...
module.exports = class Edge extends EventEmitter {
  ...
  respondOK(message, response){
    this.ws.write(message.sessionId, protocol.createReply(message.data, {status:1, response: response}));
  }

  respondError(message, e){
    this.emit('message-process-error', e);
    this.ws.write(message.sessionId, protocol.createReply(message.data, {status:0, error: protocol.serializeError(e)}));
  }
