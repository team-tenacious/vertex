  ...
  onConnection(socket) {
    ...
    jsonSocket.on('message', message => {
      if (message.action == 'join') {
        ...
        if (message.payload.secret != this.cluster.config.secret) {
        ...
        if (this.cluster.gotConnection(message.payload.address)) {
          // two nodes connecting to each other simultaneously
          // only keep one of the sockets
          this.logger.warn('%s (%s) already connected/connecting', message.payload.name, message.payload.address);

          var first = [this.cluster.advertiseAddress, message.payload.address].sort().pop();

          if (first != this.cluster.advertiseAddress) {
            return socket.end();
          }

        }

        var peer = this.cluster.addPeer({
        ...
        var peers = this.cluster.listPeers();
