...
module.exports = class SubscribeHandler extends EventEmitter {
  ...
  async process(message){
    ...
    if (await this.writeSubscriptionToCache(message) == 1) return await this.edge.cluster.request(subscriptionPeer, protocol.createMessage('cluster-subscribe', {topic:message.data.payload.topic}));
