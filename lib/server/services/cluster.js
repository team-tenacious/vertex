...
const { NoSuchPeerError } = require('../errors');
...
module.exports = class Cluster extends EventEmitter {
  ...
  async stop() {
    ...
    this.logger.error('>>>>TODO: clear all request timeouts');
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.write(toAddress, message)
        .then(() => {
          ...
          console.log('TT', this.config.requestTimeout);

          this.requests[message.tx] = [resolve, reject, timeout];
  ...
  join(address, peer) {
    ...
    peer.on('message', message => {
      if (!this.requests.hasOwnProperty(message.tx)) {
        this.emit('message', peer.address, message);
        return;
      }

      var [resolve, reject, timeout] = this.requests[message.tx];
