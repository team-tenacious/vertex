...
module.exports = {
  ...
  RequestTimeoutError: class RequestTimeoutError extends Error {
    constructor(message) {
      super(message);
      this.name = this.constructor.name;
      this.code = constants.ERRORS.SERVER.REQUEST_TIMEOUT_ERROR;
      if (typeof Error.captureStackTrace === 'function') {
        Error.captureStackTrace(this, this.constructor);
      } else {
        this.stack = (new Error(message)).stack;
  ...
  RequestToDepartedError: class RequestToDepartedError extends Error {
    constructor(message) {
      super(message);
      this.name = this.constructor.name;
      this.code = constants.ERRORS.SERVER.REQUEST_TO_DEPARTED_ERROR;
      if (typeof Error.captureStackTrace === 'function') {
        Error.captureStackTrace(this, this.constructor);
      } else {
        this.stack = (new Error(message)).stack;
